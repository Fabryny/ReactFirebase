...
/*     useEffect(() => {  
        if(props.match.params.id){                   
            firestore().collection('eventos').doc(props.match.params.id).get().then(resultado => {
...
/*     if(fotoNova)    
    ...
    function cadastrar(){
        ...
        addDoc(collection(db,'anunciamentos'), {
            titulo: titulo,
            tipo: tipo,
            detalhes: detalhes,
            data: data,
            ...
            publico: 1,
            criacao: new Date()
        }).then(() => {    console.log( 'entrooo')
            const storageRef = ref(storage, `imagens/${fotoNova.name}`)
            console.log(storageRef,'storageRerf')
            console.log(fotoNova,'fotoNova');
            uploadBytes(storageRef, fotoNova);
            setMsgTipo('sucesso');
            setCarregando(0);
        }).catch((erro) => {
            console.log(erro, 'errrrro')
        })
        

 /*        .then(() => {
            db.collection('anunciamentos').add({
                titulo: titulo,
                tipo: tipo,
                detalhes: detalhes,
                data: data,
        ...
        <div className="col-12 mt-5">
            ...
            <form>
                ...
                <div className="row">               
                {
                    ...
                    : <button onClick={cadastrar}/* props.match.params.id ? atualizar : cadastrar} */ type="button" className="btn btn-lg btn-block mt-3 mb-5 btn-cadastro">{/* {props.match.params.id ? 'Atualizar Evento' :  */}'Publicar Evento'</button>
                }
                </div>
